...
class SimulacaoServiceTest {
    ...
    private ParametroProduto parametro(double min, int maxPrazo, String risco) {
    ...
    private Simulacao simulacao(Long id, ProdutoInvestimento produto, double valorFinal, LocalDateTime dataSimulacao) {
    ...
    private SimulacaoRequest request(String tipoProduto, double valor, int prazoMeses) {
    ...
    void deveSimularInvestimentoComProdutoValido() {
        ParametroProduto parametro = parametro(1000.00, 24, "Baixo");
        ...
        SimulacaoResponse response = simulacaoService.simularInvestimento(
                request("CDB", 10000.00, 12)
        );

        assertEquals(11200.00, response.resultadoSimulacao.valorFinal, 0.01);
    ...
    void deveLancarErroQuandoTipoProdutoInvalido() {
        ...
        assertErro404(() -> simulacaoService.simularInvestimento(request("LCI", 1000.00, 12)));
    }

    @Test
    void deveLancarErroQuandoProdutoNaoEncontrado() {
        when(parametroProdutoRepository.findByRisco("CDB")).thenReturn(parametro(1000.00, 24, "Baixo"));
        when(produtoRepository.findByTipo("CDB")).thenReturn(Optional.empty());

        assertErro404(() -> simulacaoService.simularInvestimento(request("CDB", 10000.00, 12)));
    }

    @Test
    void deveLancarErroQuandoValorAbaixoDoMinimo() {
        when(parametroProdutoRepository.findByRisco("CDB")).thenReturn(parametro(5000.00, 24, "Baixo"));

        assertErro404(() -> simulacaoService.simularInvestimento(request("CDB", 1000.00, 12)));
    }

    @Test
    void deveLancarErroQuandoPrazoAcimaDoMaximo() {
        when(parametroProdutoRepository.findByRisco("CDB")).thenReturn(parametro(1000.00, 12, "Baixo"));

        assertErro404(() -> simulacaoService.simularInvestimento(request("CDB", 10000.00, 24)));
    }
    ...
    void deveListarTodasAsSimulacoesTransformandoParaDTO() {
        ...
        Simulacao sim1 = simulacao(1L, produto1, 100.00, LocalDateTime.now());
        Simulacao sim2 = simulacao(2L, produto2, 200.00, LocalDateTime.now());
    ...
    void deveGerarResumoPorProdutoEDia() {
        ...
        Simulacao sim1 = simulacao(1L, cdb, 100.00, LocalDateTime.of(2025, 11, 10, 10, 0));
        Simulacao sim2 = simulacao(2L, cdb, 200.00, LocalDateTime.of(2025, 11, 10, 11, 0));
        Simulacao sim3 = simulacao(3L, lci, 300.00, LocalDateTime.of(2025, 11, 11, 12, 0));
    ...
    void deveBuscarSimulacoesPorCliente() {
        ...
        Simulacao sim1 = simulacao(1L, produto1, 100.00, LocalDateTime.now());
        Simulacao sim2 = simulacao(2L, produto2, 200.00, LocalDateTime.now());
        Simulacao sim3 = simulacao(1L, produto2, 300.00, LocalDateTime.now());
